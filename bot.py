...
class Database:
    def __init__(self):
        ...
        if not self.settings.find_one({"_id": "api_config"}):
            self.settings.insert_one(
                {"_id": "api_config", "api_url": "", "api_key": ""}
            )
    ...
    def log_attack(
        ...
        self.users.update_one(
            {"user_id": user_id}, {"$inc": {"total_attacks": 1}}
        )
    ...
    def get_user_stats(self, user_id: int):
        ...
        success = self.attacks.count_documents(
            {"user_id": user_id, "status": "success"}
        )
        failed = self.attacks.count_documents(
            {"user_id": user_id, "status": "failed"}
        )
        recent = list(
            self.attacks.find({"user_id": user_id}).sort("timestamp", -1).limit(10)
        )
        for a in recent:
            if a.get("timestamp"):
...
# ---------- HELPERS ----------
def is_port_blocked(p):
    return p in BLOCKED_PORTS


def blocked_ports_str():
...
def start_real_attack(
    ip, port, duration, method, user_id, update_func
):
...
def main_menu(is_admin: bool, approved: bool):
    ...
    if approved:
        keyboard.append(
            [InlineKeyboardButton("🚀 Attack (API)", callback_data="attack_menu")]
        )
        keyboard.append(
            [
                InlineKeyboardButton(
                    "💀 Flood Attack (Real)", callback_data="attack_flood_menu"
                )
            ]
        )
        keyboard.append(
        ...
        keyboard.append(
            [InlineKeyboardButton("🚫 Blocked Ports", callback_data="blockedports")]
        )
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        text, parse_mode="Markdown", reply_markup=main_menu(is_admin, approved)
    )
    ...
    if data == "attack_menu":
        ...
        if s["recent"]:
            ...
            for a in s["recent"][:3]:
                ...
                txt += f"\n{'✅' if a['status']=='success' else '❌'} `{a['ip']}:{a['port']}` - `{a['duration']}s` ({ago}m ago)"
        await query.edit_message_text(txt, parse_mode="Markdown", reply_markup=main_menu(is_admin, approved))
        ...
        if not is_admin:
            ...
            return
        api_url, api_key = db.get_api_config()
        url_disp = api_url if api_url else "Not set"
        key_disp = (api_key[:10] + "...") if api_key else "Not set"
        await query.edit_message_text(
        ...
        await query.edit_message_text(
            "Main menu:", reply_markup=main_menu(is_admin, approved)
        )
        ...
        try:
            ...
            resp = requests.get(test_url, headers={"x-api-key": api_key}, timeout=10)
        ...
        if current + 10 <= 200:
            ...
            await query.edit_message_text(
                f"✅ Threads increased to `{get_attack_threads()}`",
                reply_markup=admin_panel_kb(),
            )
        else:
        ...
        if current - 10 >= 50:
            set_attack_threads(current - 10)
            await query.edit_message_text(
                f"✅ Threads decreased to `{get_attack_threads()}`",
                reply_markup=admin_panel_kb(),
            )
        else:
...
async def handle_flood_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Invalid port (1-65535, not blocked) or duration (5-300s)"
        )
        return
    ...
    msg = await update.message.reply_text(
        f"💀 Starting {method.upper()} flood on {ip}:{port} for {duration}s..."
    )
    loop = asyncio.get_running_loop()

    def update_func(text, reply_markup=None):
        asyncio.run_coroutine_threadsafe(
            msg.edit_text(text, parse_mode="Markdown", reply_markup=reply_markup),
            loop,
        )

    threading.Thread(
        target=start_real_attack,
        args=(ip, port, duration, method, uid, update_func),
        daemon=True,
    ).start()
...
async def attack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text(
        f"🚀 Launching API attack on {ip}:{port}..."
    )
...
async def blocked_ports_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def approve_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def approve_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.approve_user(uid, days):
        await update.message.reply_text(
            f"✅ User {uid} approved for {days} days."
        )
        try:
            await context.bot.send_message(
                uid, f"✅ Your account approved for {days} days!"
            )
        except:
            pass
    else:
...
async def disapprove_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = update.message.text.strip()
    if not txt.isdigit():
...
def main():
    ...
    app.add_handler(
        CallbackQueryHandler(
            button_handler,
            pattern="^(attack_menu|attack_flood_menu|flood_|myinfo|mystats|blockedports|help|admin_panel|back_main|admin_users|admin_stats|admin_status|admin_blockedports|admin_attack_settings|inc_threads|dec_threads|stop_attack|info_attack|refresh_attack)$",
        )
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "attack_menu":
        ...
        if not approved:
            await query.edit_message_text(
                "Not approved.", reply_markup=main_menu(is_admin, False)
            )['status']=='success' else '❌'} `{a['ip']}:{a['port']}` - {a['duration']}s ({ago}m ago)\n"
        await query.edit_message_text(txt, parse_mode="Markdown", reply_markup=main_menu(is_admin, approved))
    elif data == "blockedports":
        txt = f"🚫 *Blocked ports*\n{blocked_ports_str()}"
        await query.edit_message_text(txt, parse_mode="Markdown", reply_markup=main_menu(is_admin, approved))
